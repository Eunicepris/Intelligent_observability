...
st.markdown(
    "Détection multi-modale d'anomalies dans les systèmes microservices — " "**Train Ticket** et **Online Boutique**"
)
...
# Fenêtre
window = st.sidebar.text_input("Fenêtre (HH_MM)", value="08_43", help="Format : heure_minute — ex 08_43 pour 08:43")
            ...
            try:
                ...
                if r.ok:
                    ...
                    with col_m1:
                        st.metric(
                            "Métriques (LOF)",
                            "✓ Détecte" if mod["metriques"] else "○ Normal",
                        )
                    with col_m2:
                    ...
                    df_mod = pd.DataFrame(
                        [
                            {
                                "Modalité": "Métriques",
                                "Détecte": 1 if mod["metriques"] else 0,
                            },
                            {"Modalité": "Logs", "Détecte": 1 if mod["logs"] else 0},
                            {
                                "Modalité": "Traces",
                                "Détecte": 1 if mod["traces"] else 0,
                            },
                        ]
                    )
                    ...
                    if res.get("type_panne"):
                        ...
                        df_probs = pd.DataFrame(
                            [{"Type": k, "Probabilité (%)": v * 100} for k, v in tp["probabilites"].items()]
...
with tab_stats:
    ...
    try:
        ...
        if r.ok:
            ...
            df_sev = pd.DataFrame([{"Sévérité": k, "Nombre": v} for k, v in stats["par_severite"].items()])
...
st.markdown("---")
st.caption("Plateforme de détection d'anomalies — " "Projet de maîtrise en génie logiciel")
