...
class ClassificateurTypePanne:
    ...
    def __init__(self, models_dir: str = "models"):
        ...
        logger.info(f"ClassificateurTypePanne initialisé " f"(F1-weighted moyen : {self.f1_moyen*100:.1f}%)")
    ...
    def _charger_modele(self) -> None:
        ...
        if not chemin.exists():
            raise ModelError(
                f"Modèle de classification introuvable : {chemin}. " f"Vérifiez que le notebook 14 a été exécuté."
            )
        ...
        if cles_manquantes:
            raise ModelError(f"Structure du modèle invalide : clés manquantes {cles_manquantes}")
    ...
    def extraire_features(
        ...
        if systeme not in SYSTEMES_VALIDES:
            raise DataError(f"Systeme invalide : '{systeme}'. Valeurs autorisées : {SYSTEMES_VALIDES}")
    ...
    def _extraire_features_logs(
        ...
        try:
            ...
            sim = cosine_similarity(vecteur, detecteur.tfidf["vecteur_ref"].reshape(1, -1))[0, 0]
    ...
    def predire(
        ...
        resultat = {
            ...
            "confiance": confiance,
            "probabilites": {str(c): float(p) for c, p in zip(self.modele.classes_, probabilites)},
            "action_specifique": self._obtenir_action(str(type_predit)),
        }

        logger.info(f"Type prédit : {resultat['type_predit']} " f"(confiance {confiance*100:.0f}%)")
