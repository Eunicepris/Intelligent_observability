...
class PipelineComplet:
    ...
    def __init__(
        ...
        logger.info(f"Pipeline initialisé pour {systeme} (stratégie de fusion : {self.strategie})")
    ...
    def _charger_config(self, config_path: str) -> Dict[str, Any]:
        ...
        if "data" not in config or "base_path" not in config.get("data", {}):
            raise ConfigurationError(f"Configuration invalide : 'data.base_path' manquant dans {config_path}")
        if "fusion" not in config or "strategie" not in config.get("fusion", {}):
            raise ConfigurationError(f"Configuration invalide : 'fusion.strategie' manquant dans {config_path}")
    ...
    def traiter_fenetre(self, date: str, window: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Fenêtre traitée : {severite} (confiance {confiance*100:.0f}%)")
            return resultat

        except DataError as e:
            # Laisser DataError remonter à l'API pour qu'elle retourne HTTP 404
            # (au lieu de HTTP 500 comme le ferait PipelineError)
            logger.warning(f"Erreur de données pour {date} {window} : {e}")
            raise

    def traiter_batch(self, liste_fenetres: List[Tuple[str, str]]) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"Batch terminé : {len(resultats)} traitées, " f"{sum(1 for r in resultats if 'erreur' in r)} en erreur"
        )
        return resultats
