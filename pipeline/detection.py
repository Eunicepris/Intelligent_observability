...
class DetecteurAnomalies:
    ...
    def _charger_modeles(self) -> None:
        ...
        for attribut, nom_fichier in modeles_a_charger.items():
            ...
            if not chemin.exists():
                raise ModelError(
                    f"Modèle introuvable : {chemin}. " f"Vérifiez que les modèles ont été entraînés (voir notebook 12)."
                )
    ...
    def detecter_traces(self, df_traces: pd.DataFrame) -> bool:
        ...
        if anomalie:
            logger.debug(f"Anomalie traces détectée (taux {taux:.2%} > {SEUIL_IF_TRACES:.2%})")
...
def fusionner(detections: Dict[str, bool], strategie: str = "or") -> bool:
    ...
    if strategie not in STRATEGIES_VALIDES:
        raise DataError(f"Stratégie inconnue : '{strategie}'. Valeurs autorisées : {STRATEGIES_VALIDES}")
