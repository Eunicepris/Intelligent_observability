...
class SystemeAlertes:
    ...
    def __init__(self, fichier_alertes: str = "alertes.json"):
        ...
        if not self.fichier.exists():
            try:
                ...
                raise DataError(f"Impossible de créer le fichier d'alertes {self.fichier} : {e}")
    ...
    def enregistrer(self, resultat: Dict[str, Any]) -> None:
        ...
        if cles_manquantes:
            raise DataError(f"Clés requises manquantes dans le résultat : {cles_manquantes}")

        severite = resultat.get("severite")
        if severite not in SEVERITES_VALIDES:
            raise DataError(f"Sévérité invalide '{severite}'. Valeurs autorisées : {SEVERITES_VALIDES}")
        ...
        logger.info(f"Alerte {severite} enregistrée : {alerte['systeme']} / {alerte['fenetre']}")
    ...
    def obtenir(
        ...
        if severite is not None and severite not in SEVERITES_VALIDES:
            raise DataError(f"Filtre severite invalide '{severite}'. Valeurs autorisées : {SEVERITES_VALIDES}")
    ...
    def _sauvegarder(self, alertes: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            raise DataError(f"Impossible d'écrire dans le fichier d'alertes {self.fichier} : {e}")
