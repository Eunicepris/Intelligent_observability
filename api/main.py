...
async def log_requests(request: Request, call_next):
    ...
    logger.info(f"{request.method} {request.url.path} " f"→ {reponse.status_code} ({duree_ms:.0f}ms)")
    return reponse
...
def detecter_anomalie(requete: RequeteDetection) -> Dict[str, Any]:
    ...
    if requete.systeme not in pipelines:
        raise HTTPException(
            ...
            detail=(f"Système inconnu : {requete.systeme}. " f"Systèmes disponibles : {list(pipelines.keys())}"),
        )
    ...
    try:
        ...
        # Entrée utilisateur mal formée (ex: format de fenêtre invalide)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Entrée invalide : {e}",
        )
    except DataError as e:
        # Donnée introuvable ou manquante (ex: fenêtre absente du dataset)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Données introuvables : {e}",
        )
